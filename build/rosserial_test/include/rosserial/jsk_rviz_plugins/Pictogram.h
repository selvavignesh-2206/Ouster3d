...
namespace jsk_rviz_plugins
{
  ...
  class Pictogram : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      u_ttl.base = 0;
      u_ttl.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_ttl.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_ttl.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_ttl.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
      this->ttl = u_ttl.real;
      offset += sizeof(this->ttl);
      union {
        double real;
