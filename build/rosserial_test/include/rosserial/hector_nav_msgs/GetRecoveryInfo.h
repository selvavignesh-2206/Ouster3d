...
static const char GETRECOVERYINFO[] = "hector_nav_msgs/GetRecoveryInfo";
  ...
  class GetRecoveryInfoRequest : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      u_request_radius.base |= ((uint32_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_request_radius.base |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_request_radius.base |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_request_radius.base |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3);
